        ...
        try {
            ...
            if (path.startsWith('/api/')) {
                ...
                if (subscriptionMatch) {
                    ...
                    if (method === 'PUT') return await updateSubscription(request, env, id);
                    if (method === 'DELETE') return await deleteSubscription(request, env, id);
                }
                ...
                if (apiPath === '/email/test' && method === 'POST') {
                    try {
                        ...
                        return errorResponse(`发送测试邮件时发生错误: ${errorMessage}`, 500);
                    }
                }
