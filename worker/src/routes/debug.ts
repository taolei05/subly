...
export async function getNotifyStatus(
	...
	const resendLastSent = resendConfig?.last_sent_at
		...
		: null;
	const serverchanLastSent = serverchanConfig?.last_sent_at
		? new Date(serverchanConfig.last_sent_at)
		: null;
...
export async function forceNotify(
  ...
  `)
	...
	if (type === "all" || type === "email") {
		const resendConfig = await env.DB.prepare(
			"SELECT email, api_key, domain FROM resend_config WHERE user_id = ?",
		)
			.bind(payload.userId)
...
export async function resetLastSent(
	...
	if (type === "all" || type === "email") {
		await env.DB.prepare(
			"UPDATE resend_config SET last_sent_at = NULL WHERE user_id = ?",
		)
			.bind(payload.userId)
			.run();
	}
	...
	if (type === "all" || type === "serverchan") {
		await env.DB.prepare(
			"UPDATE serverchan_config SET last_sent_at = NULL WHERE user_id = ?",
		)
			.bind(payload.userId)
			.run();
	}
...
function generateServerChanContent(
